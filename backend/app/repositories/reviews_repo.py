...
def _dict_to_row(data: Dict[str, Any]) -> Dict[str, str]:
    ...
    try:
        created_dt = datetime.fromisoformat(created_iso) if created_iso else datetime.now(timezone.utc)
    except Exception:
    ...
    return {
        ...
        "User's Rating out of 10": data.get("rating", "") if data.get("rating") is not None else "",
    ...
    def list_by_movie(
        ...
        with open(path, newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                if min_rating is not None and (d["rating"] is None or d["rating"] < min_rating):
                    continue
