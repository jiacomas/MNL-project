...
def test_pagination(seeded_repo):
    ...
    assert len(page1) == 1 and cur1 == 1 # cur1 should point to the second item (index 1)
    
    page2, cur2 = repo.list_by_movie(movie_id, limit=1, cursor=cur1)
    assert len(page2) == 1 and cur2 is None # cur2 should be None as there are no more items
